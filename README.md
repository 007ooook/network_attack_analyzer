...
仪表盘提供系统总览，包括核心统计指标、攻击趋势面积图、攻击分布饼图、威胁情报摘要和最近告警/日志列表。

### 日志分析

![日志分析](screenshots/log-analysis.png)

深度日志分析中心，支持格式/严重程度筛选，展示日志格式分布、攻击类型分布饼图、24h 攻击趋势折线图。

### 威胁情报

![威胁情报](screenshots/threat-intel.png)

威胁情报管理，展示威胁情报列表，支持一键拉取最新情报，展示情报来源和类型分布。

### 告警管理

![告警管理](screenshots/alerts.png)

安全告警中心，支持严重程度筛选（Critical/High/Medium/Low/Info），分页告警列表，去重/聚合状态标记。

### 规则管理

![规则管理](screenshots/rules.png)

告警规则管理，支持添加/编辑/删除规则，规则条件 JSON 编辑器，启用/禁用开关。

### 数据源管理

![数据源管理](screenshots/data-sources.png)

数据源配置中心，支持 9 种数据源类型的添加/编辑/删除/启停/测试，实时统计刷新。

### 日志解析

![日志解析](screenshots/log-parser.png)

在线日志解析工具，支持输入原始日志文本，AI 自动识别日志格式，提取攻击类型和威胁等级。

### 日志列表

![日志列表](screenshots/logs.png)

原始日志数据浏览，分页查询，支持攻击类型/威胁等级标记，日志详情弹窗。

### 攻击预测

![攻击预测](screenshots/prediction.png)

基于历史数据的攻击预测，展示 7 天攻击趋势预测折线图，预测详情列表，30 秒自动更新。

### 数据源日志

![数据源日志](screenshots/data-source-logs.png)

按数据源查看日志，支持数据源筛选、时间段选择、关键词搜索、日志详情弹窗。

### 系统配置

![系统配置](screenshots/system-config.png)

系统设置中心，包括安全配置（密码复杂度、登录锁定、会话超时）、API 配置、服务配置。
